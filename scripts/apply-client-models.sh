#!/usr/bin/env bash
# Deploy simulator models and MaaS CRs on client clusters.
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage: apply-client-models.sh [--kubeconfig PATH]

Deploys free + premium simulator bundles (LLMInferenceService, MaaSModelRef,
MaaSAuthPolicy, MaaSSubscription). Subscription names must match the hub.
EOF
}
...
log "Deploying client models..."
kustomize build "${POC_ROOT}/kustomize/client/models" | oc apply -f -
